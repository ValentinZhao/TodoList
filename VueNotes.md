...
Observer绑定data过程大致是，在生命周期中会调用一个vm._initData()方法来处理data选项。在里面又会有遍历data的key的方法`_proxy()`，它会遍历data的key通过Object.defineProperty方法来代理getter，setter。最后在init中会调用`observe(data, this)`方法来创建一个Observer，那么Observer做的事情是：创建一个`Dep`对象实例，然后对传进来的data进行类型判断（单个对象还是数组），对数组也是递归调用observe，最终会调用`walk`方法观察单元素。在walk中，对单元素的key进行遍历，对每个属性执行`convert`方法。convert方法再调用`defineReactive`方法，这个方法就是为每个属性添加setter，getter，以此来实现了数据劫持。当某个属性被访问时，属性的getter会被调用——判断当`Dep.target`不为空时，调用`dep.depend`和`childObj.dep.depend`做依赖收集。改变data属性时，则会调用setter方法，这时会调用dep.notify方法进行通知了。<br>
那么提到这个Dep，它是一个简单的观察者实现，维护着一个subs数组，subs存储所有订阅它的watcher，`Dep.target`代表正在计算的watcher，全局唯一。那么，getter和setter在触发时，其实是分别调用了`dep.depend`和`dep.notify`方法。这两个方法很简单，depend的底层实现就是把当前的这个target给加入到subs中，notify则是遍历订阅的watcher，也就是subs数组分别调用`update`方法。这样就做到了给data对象添加Observer，因为你对数据操作我们都有相应的触发方法了。<br>
那么接下来是编译，绑定指令和watcher。通过`transclude`方法把一段template来编译成DOM fragment，指令解析部分通过`compile(el,options)`来实现。compile其实也是compileNodes方法的封装，遍历递归DOM树的节点。在遍历节点的过程中会根据节点的类型来判断如何渲染，最后会append到一个fragment中。在创建中有一个`processTextNode()`方法为节点来拓展出一个token，这个token就是记录其各种信息的对象，之后的绑定会解析其token中的字段解析directives，来添加到directives数组完成命令解析。directives在初始化时创建了`this._update()`方法，并创建了watcher，把`this._update`方法作为watcher的回调函数。这下watcher观测到的数据变动，就会调用directives的`_update()`方法，然后相应指令的部分进行编译最终调用本部分的update方法实现DOM刷新。<br>
最后看一下watcher是怎么串联起整个流程的。watcher作为订阅者，其中关联起数据变化和directives视图渲染的关键一步就是`Watcher.prototype.beforeGet()`之后的`this.getter.call(scope,scope)`方法。beforeGet方法只是将Dep.target作为当前的观察者。传入scope，就是我们当前的Vue实例this.vm，那么这样就触发了getter。之后通过`addSub`等一些列的工作完成了watcher的依赖添加，subs数组中就都是订阅的watcher了。那么当有数据变动时，setter会触发`dep.notify`方法，这个方法会遍历watcher依次触发他们的update方法。最后会调用`watcher.run()`方法，这个run方法是对watcher求值，如果数据变化会触发`this.cb.call(this.vm,value,oldValue)`，这个callback是当初Directives实例创建watchers传入的回调方法，就又会进入directives的渲染DOM的方法中完成视图刷新了。