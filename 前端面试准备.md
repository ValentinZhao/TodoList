...
bind方法与上面两个的区别在于，bind调用之后只是把this改变，返回的依然是函数，而上面两个调用之后方法也会被触发。
```
...
[一篇相当不错的bind文章](https://www.smashingmagazine.com/2014/01/understanding-javascript-function-prototype-bind/)
...
而是先向客户端发送一个ACK=1的应答信息，表示：你请求关闭的请求我已经收到，但我可能还有数据没有完成传送，你再等下，等我数据传输完成了我就告诉你；
  ...
  - 第四次挥手：客户端收到FIND=1后，客户端还是怕由于网络不稳定的原因，怕服务端不知道他要断开连接，于是向服务端发送ACK=1确认信息进行确认，把自己设置成TIME_WAIT状态并启动定时器，如果服务端没有收到ACK，服务端TCP的定时器到达后，会要求客户端重新发送ACK，当服务端收到ACK后，服务端就断开连接；
...
- setTimeout精准定时
setTimeout和setInterval这类方法的延时，其实是把事件塞到实践队列的尾部等待执行，所以并不能做到精准定时。可以把相应事件放到node中执行，process.nextTick()方法可以把指定的事件放在正在执行队列的最后，等待队列的前面。

- 函数的节流（throttle）

背景：比如为页面绑定scroll、resize事件，在一个正常的操作中可能会频繁的出发多次事件绑定程序，而DOM操作很消耗性能，很有可能导致浏览器崩溃

所以函数的节流就是让一个函数无法在很短的时间间隔内连续调用，只有当上一次函数执行后，经过规定的时间间隔，才能进行下一次调用。

原理：利用定时器setTimeout（发现setTimeout是一门玄学。。。），当触发一个事件时，先setTimeout让这个事件延迟一会儿在执行，如果在这个间隔内又触发了事件，那我们就clear原来的定时器，再setTimeout一个新的定时器，延迟一会儿再执行。
...
var throttle = function(fn, delay){
        ...
        //call
window.onresize = throttle(myfn, 100);
```
- node模块加载顺序
...
[HTTP headers](https://kb.cnblogs.com/page/92320/)
...
3. 通信流程
![](https://uploadfiles.nowcoder.com/files/20160727/213669_1469604624728_cgiarch.gif)
4. 消息结构
...
- Websocket协议，DNS协议
 - DNS基础和解析
 1. 根域：就是所谓的“.”，其实我们的网址www.baidu.com在配置当中应该是www.baidu.com.（最后有一点），一般我们在浏览器里输入时会省略后面的点，而这也已经成为了习惯。
...
比如a.www.baidu.com，在这个网址中，www.baidu.com变成了一个二级域而不是一台主机，主机名是a。
 3. 域名服务器：能提供域名解析的服务器，上面的记录类型可以是A(address)记录，NS记录（name server），MX（mail），CNAME等。A记录是什么意思呢，就是记录一个IP地址和一个主机名字，比如我这个域名服务器所在的域test.baidu.com，我们知道这是一个二级的域名，然后我在里面有一条A记录,记录了主机为a的IP，查到了就返回给你了。
如果我现在要想baidu.com这个域名服务器查询a.test.baidu.com，那么这个顶级域名服务器就会发现你请求的这个网址在test.baidu.com这个域中，我这里记录了这个二级域的域名服务器test.baidu.com的NS的IP。我返回给你这个地址你再去查主机为a的主机把。这些域内的域名服务器都称为权威服务器，直接提供DNS查询服务。（这些服务器可不会做递归哦）
 4. 解析过程：1.现在我有一台计算机，通过ISP接入了互联网，那么ISP就会给我分配一个DNS服务器，这个DNS服务器不是权威服务器，而是相当于一个代理的dns解析服务器，他会帮你迭代权威服务器返回的应答，然后把最终查到IP返回给你。
...
>6、如果用的是转发模式，此DNS服务器就会把请求转发至上一级DNS服务器，由上一级服务器进行解析，上一级服务器如果不能解析，或找根DNS或把转请求转至上上级，以此循环。不管是本地DNS服务器用是是转发，还是根提示，最后都是把结果返回给本地DNS服务器，由此DNS服务器再返回给客户机。
 ...
 websocket的诞生场景是因为我们在浏览器上缺少一种与服务器保持长连接的标准化的技术，因为HTTP 1.1（以下简称为HTTP）协议只是一个标准的无状态协议，并不存在除了request和response生命周期之外的通信场景。如果我们需要实现在线聊天的功能的话，那么基于HTTP，我们只能使用丑陋的long polling和ajax 轮询等非正常的技术实现我们需要的功能。这些方案虽然解决了我们的使用场景，但并不是最好的方案，我们的HTTP服务器软件并不是设计来保持长连接的。基于以上场景，websocket诞生了。
 
>它是一种真正的长连接，能让服务端和客户端进行持久的通信，轻松的实现服务器推送技术。
>
>它和HTTP并没有特别多的关系，但是它的握手 (handshake) 是利用HTTP来完成。
>
>它的本质是基于TCP的连接，可以说和HTTP属于平级的应用层协议。

除了建立起基于TCP的一层连接以外，还有以下几个特点
...
websocket的握手实际上就是给服务器发送一个GET请求，里面带上指定的header即可。request例子如下：
...
![](https://segmentfault.com/img/bVxZFC?w=554&h=293)
...
在websocket中，我们定义了几种操作类型，也就是表明了数据包的行为，数据包大体可分为两种，一种是字符数据包 (string)，一种是字节数据包 (byte) 不同的数据包使用不同的opcode来传输，opcode定义如下：首先Opcode占用4bit的长度
...
如果是客户端发送到服务端的数据包，我们需要使用掩码对payload的每一个字节进行异或运算，生成masked payload 才能被服务器读取。具体的运算其实很简单
...
构建dom树的过程即根据html代码自上而下进行构建，当遇到script文件加载／执行会阻塞后面dom树的构建（javascript可能会改变dom树），而遇到css文件则会阻塞渲染树的构建，即dom树依然继续构建（除非遇到script标签并且css文件依旧未加载完成），但不会渲染绘制到页面上。而无论哪个阻塞，该加载的文件还是会加载，例如html文档中的其他css／js／图片文件。另外javascript被加载后就会被执行，执行的过程也阻塞树的构建。是执行完了才解析其他内容，而不是执行完了才加载其他内容。        至于asyn和defer，其实上面说的script阻塞就可以用它们解决了，详细自行查找资料。
...
主要就是会把我们在原型上注册的属性也给遍历出来即使有时我们不需要。所以我们通常结合`hasOwnProperty`方法先判断该key是不是在原型上的，在选择是否遍历它

- JavaScript事件循环？
- 同步异步与async/await的区别


由于浏览器/引擎负责解释和执行JavaScript的主线程是单线程，同步执行一个耗时较大的任务会导致阻塞。异步执行代码可以解决阻塞问题，但会带来顺序的不确定性（要注意这里顺序的不确定性指的是多个异步过程的不确定性）。如果这些任务彼此不相关，就不一定需要交互，如果没有相互影响的话，不确定性是完全可以接受的。如果你需要保证异步执行的顺序，比如依次远程读取一系列url, 按顺序触发动画等，就需要嵌套很多层回调函数。多级嵌套的回调函数弊端很多：不直观，强耦合，回调的不确定性，不利于维护与复用。Promise对象允许将这种回调函数的嵌套，改为链式调用。并用then保证执行顺序。Promise还能保证每次返回的都是一个新的Promise对象，所以代码一定被异步执行。这种方法还是不够简明，语义化，你只能看见一堆then。于是有了async和await，把异步执行的代码写得像同步代码那样直观。async的函数中可以有一个或多个异步操作，一旦遇到await就会立即返回一个pending状态的Promise对象，暂时返回执行代码的控制权，使得**函数外**(也就是说async函数确实是阻塞了的)的代码得以继续执行，这是保证非阻塞的部分。并且多个异步请求是可以并发的（多个Job Queue、Ajax、Timer、I/O(Node)）。等await后的异步请求resolve了（或reject)，主线程才会继续执行async函数内后面的部分，这是保证的顺序性（而且最终函数返回的也是一个Promise对象）。我们说的把异步变同步指的是在async函数内部异步代码就像被同步执行的那样（继发执行），而不是它会阻塞主线程一直等待异步调用返回。

![](https://pic1.zhimg.com/v2-3ff74dd1104dfc9c10fb217b6b8cc034_b.jpg)

第一个输出的false是由于执行到synchronous函数中同步代码的时候异步的过程还未返回；第二个的undefined是因为这个函数没有返回值，默认是undefined；第三个Promise是由于执行到asynchronous函数中遇到await就返回了，返回值是一个Promise，并继续执行后续其他任务；于是输出的第四个值是最后那句长长的字符串；await等到它promise的结果后（可能是resolve，或者reject）通知主线程，主线程执行完当前所有 chunk 后，就回到当初暂停的地方继续执行asynchronous函数；于是最后打印出true。另外这个通知过程上面的synchronous函数也有，只不过通知完并不需要处理任何任务这里就略过了。如果把asynchronous函数换成一个同步的函数的话，那最后加上的长字符串会最后输出，假如你在这个函数内发出的同步请求耗时很长，后面的代码和函数外的代码都只能干等着。